...
@pytest.mark.web
def test_OH_WEB_026_leave_list_loads_on_navigation(logged_in_page, orangehrm_base_url):
...
@pytest.mark.web
def test_OH_WEB_027_leave_list_shows_record_count(logged_in_page, orangehrm_base_url):
    leave = LeavePage(logged_in_page, orangehrm_base_url)
    leave.navigate_to_list()
    leave.click_search()
...
@pytest.mark.web
def test_OH_WEB_028_filter_by_pending_status_shows_results(logged_in_page, orangehrm_base_url):
    leave = LeavePage(logged_in_page, orangehrm_base_url)
    leave.navigate_to_list()
...
@pytest.mark.web
def test_OH_WEB_029_search_with_future_date_range_shows_no_records(logged_in_page, orangehrm_base_url):
    leave = LeavePage(logged_in_page, orangehrm_base_url)
    leave.navigate_to_list()
...
def test_OH_WEB_030_date_to_before_date_from_shows_empty_or_error(logged_in_page, orangehrm_base_url):
    leave = LeavePage(logged_in_page, orangehrm_base_url)
    leave.navigate_to_list()
